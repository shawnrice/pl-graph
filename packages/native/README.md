...
- `createIndex({ on, kind, keys })` — e.g. `{ on: 'vertex', kind: 'hash', keys: ['id'] }` (+ `drop*Index`, `vertexIndexes()` / `edgeIndexes()`) — opt-in property indexes; a `WHERE v.k = $x` / inline `{k: $x}` point lookup then seeks instead of scanning. `kind: 'interval'` builds an edge RI-tree for temporal as-of / overlap seeks. Host-API only (no GQL `CREATE INDEX`).
