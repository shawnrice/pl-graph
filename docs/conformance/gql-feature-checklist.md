...
| Map / record values (GV45) — 🟡 partial                           | GQL record value shipped both engines (construct `{k:v}`, `.field`/`['k']` access, equality/ordering/DISTINCT, SET); stored maps + dotted-path index in the native substrate. Remaining: TS-core stored-map persistence + the Gremlin read boundary. |
