...
- **IS LABELED + label expressions** (~14: m*is_labeled_tests_element*\*, is_labeled, m_is_not_labeled_negates, m_label_conjunction `(n:Person&Software)`, m_label_negation `(n:!Software)`, m_label_wildcard `(n:%)`, m_is_as_label_introducer `(n IS Person)`, m_edge_label_negation `-[:!CREATED]->`, m_colon_label_predicate `x:Person|Software`). `IS LABELED L` / `IS NOT LABELED L` predicate is the tractable core (desugar to the existing label-predicate `x:L` → In(labels(x))). The full label ALGEBRA (&/|/!/%) in node/rel position is bigger; `&` conjunction and `!` negation are the next steps. Node-label disjunction `(n:A|B)` was already noted deferred but composes here.
