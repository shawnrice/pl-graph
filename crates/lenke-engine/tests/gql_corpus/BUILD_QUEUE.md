...
- **Label algebra in pattern position** (~9): `(n:A&B)` conjunction (AND of label filters — seed Scan on first, filter the rest), `(n:!A)` negation, `(n:%)` wildcard (any label), `(n IS Person)` IS-introducer (= `:Person`), edge `-[:!T]->` negation, label disjunction in a WHERE predicate (`x:A|B`). Medium; the `&` conjunction and `IS` introducer are the most tractable.
