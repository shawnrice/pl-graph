...
/// Recognize a single-key tagged-temporal object `{"@<kind>":"<iso>"}` (the wire
/// form emitted by the JSON-family codecs), returning `(tag, iso)` with the `@`
/// stripped. `None` if `pairs` isn't that shape — the caller then treats the
/// object as a map. The ISO string is NOT validated here (the host validates when
